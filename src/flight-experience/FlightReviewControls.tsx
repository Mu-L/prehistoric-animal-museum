...
export function FlightReviewControls({ runtime }: { runtime: FlightRuntime }) {
  ...
  return <details className="flight-review" data-lookdev-active={lookdev}><summary>{lookdev?'材质与植被预览':'Visual diagnostics'}</summary>
    <label>Main terrain diagnosis<select aria-label="Main terrain diagnosis" defaultValue={String(runtime.surfaceReview.value.x)} onChange={e=>{runtime.setSurfaceReview(Number(e.target.value),runtime.surfaceReview.value.y);render(n=>n+1)}}><option value="0">PBR</option><option value="1">Six surface IDs</option><option value="2">One-hot source layer</option><option value="3">Weight grayscale</option><option value="4">Unlit albedo</option><option value="5">Tangent normal</option><option value="6">Roughness</option><option value="7">Shading normal (view)</option><option value="8">Indirect diffuse</option><option value="9">Direct diffuse</option></select></label>
    <label>Diagnostic layer<select aria-label="Diagnostic layer" defaultValue="0" onChange={e=>{runtime.setSurfaceReview(runtime.surfaceReview.value.x,Number(e.target.value));render(n=>n+1)}}>{['rock PBR','soil PBR','sand PBR','mud PBR','organic PBR','groundcover PBR','rock semantic','soil semantic','beach semantic','mud semantic','organic semantic','groundcover semantic'].map((name,i)=><option key={name} value={i}>{name}</option>)}</select></label>
