import {captureCameraCandidates,recordCameraTransitions,recordCameraFlight,recordFlightEffort} from './camera-review'
...
export function FlightReviewControls({ runtime }: { runtime: FlightRuntime }) {
  ...
  return <details className="flight-review" data-lookdev-active={lookdev}><summary>{lookdev?'材质与植被预览':'Visual diagnostics'}</summary>
    <button type="button" disabled={recording} onClick={()=>{setRecording(true);void captureCameraCandidates(runtime).then(setSavedTrace).catch(e=>setSavedTrace(String(e))).finally(()=>setRecording(false))}}>Save C1 six-view PBR</button>
    <button type="button" disabled={recording} onClick={()=>{setRecording(true);void recordCameraTransitions(runtime).then(setSavedTrace).catch(e=>setSavedTrace(String(e))).finally(()=>setRecording(false))}}>Record C1 transitions</button>
    <button type="button" disabled={recording} onClick={()=>{setRecording(true);void recordCameraFlight(runtime).then(setSavedTrace).catch(e=>setSavedTrace(String(e))).finally(()=>setRecording(false))}}>Record C1 original flap</button>
    <button type="button" disabled={recording} onClick={()=>{setRecording(true);void recordFlightEffort(runtime).then(setSavedTrace).catch(e=>setSavedTrace(String(e))).finally(()=>setRecording(false))}}>Record climb and descent</button>
