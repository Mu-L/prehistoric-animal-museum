...
export function LightViewpointPanel({runtime,snapshot,locale,onClose}:{runtime:FlightRuntime;snapshot:FlightSnapshot;locale:'en'|'zh-CN';onClose:()=>void}) {
 ...
 return <section className="flight-light-panel" aria-label={t.title}>
  <div className="flight-card__heading"><strong>{t.title}</strong><button type="button" onClick={onClose}>{t.close}</button></div>
  ...
  {phase!=='inactive'&&<><p role="status">{phase==='failed'?t.failed:preparing?phase==='returning'?t.returning:t.preparing:t.active}</p><div className="flight-light-options">
