...
export function decorateMaterialTerrain(material:MeshStandardMaterial,library:MeshStandardMaterial[],trial:MaterialTrial,origin?:{value:Vector2},review?:{value:Vector2}){
 ...
 material.onBeforeCompile=(s,r)=>{
  ...
  const declaration=`uniform sampler2D trialAlbedoAtlas; uniform sampler2D trialNormalAtlas; uniform sampler2D trialARMAtlas;\n`+library.map((_,i)=>`uniform float trialScale${i}; uniform vec4 trialAtlasRect${i};`).join('\n')
...
vec4 trialCell(sampler2D tex,vec4 rect,vec2 p,vec2 dx,vec2 dy,vec2 cell,bool bump,bool directional){float a=directional?0.0:trialHash(cell)*6.2831853;float c=cos(a),s=sin(a);mat2 rotation=mat2(c,s,-s,c);vec2 uv=rotation*p+vec2(trialHash(cell+23.7),trialHash(cell+71.3))*17.0;vec4 sampleValue=textureGrad(tex,rect.xy+fract(uv)*rect.zw,rotation*dx*rect.zw,rotation*dy*rect.zw);if(bump){vec3 n=sampleValue.xyz*2.0-1.0;n.xy=transpose(rotation)*n.xy;return vec4(n,1.0);}return sampleValue;}
vec4 trialSample(sampler2D tex,vec4 rect,vec2 p,vec2 dx,vec2 dy,bool bump,bool gaussian,bool directional){
 #if TRIAL_METHOD == 0
 vec4 direct=textureGrad(tex,rect.xy+fract(p)*rect.zw,dx*rect.zw,dy*rect.zw);return bump?vec4(direct.xyz*2.-1.,1.):direct;
 #elif TRIAL_METHOD == 1
 vec2 base=floor(trialPosition.xz/12.),f=smoothstep(vec2(0),vec2(1),fract(trialPosition.xz/12.));
 return mix(mix(trialCell(tex,rect,p,dx,dy,base,bump,directional),trialCell(tex,rect,p,dx,dy,base+vec2(1,0),bump,directional),f.x),mix(trialCell(tex,rect,p,dx,dy,base+vec2(0,1),bump,directional),trialCell(tex,rect,p,dx,dy,base+vec2(1,1),bump,directional),f.x),f.y);
 #else
 ...
 vec4 value=trialCell(tex,rect,p,dx,dy,a,bump,directional)*w.x+trialCell(tex,rect,p,dx,dy,b,bump,directional)*w.y+trialCell(tex,rect,p,dx,dy,c,bump,directional)*w.z;
 if(gaussian)value.rgb=(value.rgb-.5)/sqrt(dot(w,w))+.5;
 return value;
 #endif
}
vec3 trialColor(sampler2D tex,sampler2D inverse,vec4 rect,vec2 p,vec2 dx,vec2 dy,bool directional,float layer){
 #if TRIAL_METHOD != 3
 return trialSample(tex,rect,p,dx,dy,false,false,directional).rgb;
 #else
 vec3 g=clamp(trialSample(tex,rect,p,dx,dy,false,true,directional).rgb,0.,1.);
 float level=clamp(log2(max(length(dx),length(dy))*512.),0.,9.);
 vec3 srgb=vec3(texture2D(inverse,vec2(g.r,1.-(level+.5+layer*10.)/${library.length*10}.)).r,texture2D(inverse,vec2(g.g,1.-(level+.5+layer*10.)/${library.length*10}.)).g,texture2D(inverse,vec2(g.b,1.-(level+.5+layer*10.)/${library.length*10}.)).b);
...
`)
  ...
  const layers=library.map((_,i)=>{const weight=i<4?`tw.${'xyzw'[i]}`:origin?`tx.${'xy'[i-4]}`:'0.';const grade=i===0?'color=mix(vec3(.21,.15,.11),color,.38);':i===5?'color*=vec3(.55,1.12,.93);':'';return `if(${weight}>.001){vec2 p=trialUV/trialScale${i},dx=trialDx/trialScale${i},dy=trialDy/trialScale${i};float weight=${weight};vec3 color=trialColor(trialAlbedoAtlas,trialInverse,trialAtlasRect${i},p,dx,dy,${i===0||i===2?'true':'false'},${i}.0);${grade}trialC+=color*weight;trialA+=trialSample(trialARMAtlas,trialAtlasRect${i},p,dx,dy,false,false,${i===0||i===2?'true':'false'}).rgb*weight;trialN+=trialSample(trialNormalAtlas,trialAtlasRect${i},p,dx,dy,true,false,${i===0||i===2?'true':'false'}).xyz*weight;}`}).join('\n')
  s.fragmentShader=s.fragmentShader.replace('#include <map_fragment>',`vec3 tn=normalize(trialNormal);vec2 trialUV=abs(tn.y)>=max(abs(tn.x),abs(tn.z))?trialPosition.xz:(abs(tn.x)>abs(tn.z)?trialPosition.zy:trialPosition.xy);vec2 trialDx=dFdx(trialUV),trialDy=dFdy(trialUV);vec4 tw=trialWeights();vec2 tx=${origin?'trialExtraWeights()':'vec2(0.)'};${trial.layers===2?'tw=vec4(tw.x,1.-tw.x,0.,0.);':''}${origin?'float rockEdge=tw.x;float edgeMask=smoothstep(0.,.22,rockEdge)*(1.-smoothstep(.78,1.,rockEdge));float patches=(trialNoise(trialPosition.xz/34.)-.5)*.67+(trialNoise(trialPosition.xz/10.)-.5)*.33;tw.x=clamp(rockEdge+patches*.72*edgeMask,0.,rockEdge+tw.y);tw.y+=rockEdge-tw.x;':''}${review?'if(surfaceReview.x==2.){int layer=int(surfaceReview.y);tw=vec4(float(layer==0),float(layer==1),float(layer==2),float(layer==3));tx=vec2(float(layer==4),float(layer==5));}':''}vec3 trialC=vec3(0),trialA=vec3(0),trialN=vec3(0);${layers}diffuseColor.rgb*=trialC;`)
