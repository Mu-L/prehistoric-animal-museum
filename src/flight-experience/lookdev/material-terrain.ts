...
`)
  ...
  const layers=library.map((_,i)=>{const weight=i<4?`tw.${'xyzw'[i]}`:origin?`tx.${'xy'[i-4]}`:'0.';const grade=i===0?`color=mix(vec3(.21,.15,.11),color,.38);${origin?'float stratum=trialNoise(vec2(trialPosition.x*.016+trialPosition.z*.010,trialPosition.y*.105));float stonePatch=trialNoise(trialPosition.xz/48.);color*=.82+.30*stratum+.12*stonePatch;':''}`:i===5?'color*=vec3(.72,.94,.82);':'';const fadeStart=i===1?.005:.014,fadeEnd=i===1?.02:.065;return `if(${weight}>.001){vec2 p=trialUV/trialScale${i},dx=trialDx/trialScale${i},dy=trialDy/trialScale${i};float weight=${weight};vec3 color=trialColor(trialAlbedoAtlas,trialInverse,trialAtlasRect${i},p,dx,dy,${i===0||i===2?'true':'false'},${i}.0,${i}.0);${origin?`// The source spans only a few metres; at flight height its repeated features must not become landscape-scale relief.
