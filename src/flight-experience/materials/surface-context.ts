...
export interface SurfaceContext {
  ...
  woodland: number; geology: number; water: { valid: boolean; kind: 'river' | 'sea' | 'none'; distance: number; relativeLevel: number }
...
export function surfaceContext(world:WorldSampler,x:number,z:number):SurfaceContext {
  ...
  return {worldKey:`${world.config.seed}:${world.config.generator}:${world.config.preset}`,materialVersion:MATERIAL_VERSION,
    height:t.height,slope:1-n[1],moisture:t.moisture,woodland:world.noise(x/240,z/240,71),geology:t.canyonWeight,
...
export function classifySurface(c:SurfaceContext):SurfaceClassification {
  ...
  const rock=clamp(smooth(.10,.36,slope)+finite(c.geology)*.18,0,1)
  ...
  const cover=smooth(.46,.72,moisture)*(1-smooth(.56,.78,woodland))*(1-bank)*.65
