...
export class FlightScenery {
  ...
  readonly review = { hideProps: false, flatWater: false, freezeWater: false, oceanEdges: false, skyColors: false, shadows: true, freezeBathymetry:false }
  ...
  constructor(scene: Scene, wake: () => void, surface: (x: number,z: number)=>number = (x,z)=>createWorldSampler().terrainAt(x,z).height, world = createWorldSampler(), farSurface: (x:number,z:number)=>number = surface) {
    ...
    this.river = new RiverScene(world, material=>this.environment.fog.decorate(material), wake);this.root.add(this.river.root)
  ...
  update(x:number,y:number,z:number,time:number,quality:'low'|'balanced',camera?:PerspectiveCamera, options:EnvironmentUpdateOptions & PropFrameContext & {propsFirst?:boolean;canopyFirst?:boolean} = {}) {
    this.fallbackCamera.position.set(x-this.origin.x,y,z-this.origin.z)
    Object.assign(this.environment.review,{flatWater:this.review.flatWater,freezeWater:this.review.freezeWater,oceanEdges:this.review.oceanEdges,skyColors:this.review.skyColors,shadows:this.review.shadows,freezeBathymetry:this.review.freezeBathymetry})
    ...
    const props=()=>this.props.update(x,y,z,quality,{...options,camera:camera??this.fallbackCamera})
    const canopy=()=>this.farCanopy.update(camera??this.fallbackCamera,this.origin,quality,options.budget)
