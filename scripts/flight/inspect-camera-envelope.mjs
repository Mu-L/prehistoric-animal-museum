import {flightSpecies} from '../../src/flight-experience/species/profiles.ts'
...
const id=process.argv[2]??'pteranodon',profile=flightSpecies(id)
if(!profile)throw new Error('Unsupported species')
const path=`src/content/animals/${id}/model/model.glb`,source=await readFile(path),doc=await io.read(path)
...
const initial=new Box3().setFromObject(g.scene,true),size=initial.getSize(new Vector3()),center=initial.getCenter(new Vector3()),scale=profile.spanMeters/(profile.spanAxis==='z'?size.z:Math.max(size.x,size.z))
...
for(const aspect of [320/844,390/844,844/390,16/9])for(const view of ['near','standard','wide'])for(const degrees of [0,150,165,180,-90,90]){
 const camera=new PerspectiveCamera(55,aspect,profile.camera.near,24000),pose=orbitPose({x:0,y:190,z:0},0,{yaw:degrees*Math.PI/180,pitch:0},aspect,view,profile.camera)
 ...
 for(let i=0;i<=120;i++){
  ...
  for(const x of [box.min.x,box.max.x])for(const y of [box.min.y,box.max.y])for(const z of [box.min.z,box.max.z]){
   const point=new Vector3(x,y,z).sub(center).multiplyScalar(scale);point.applyAxisAngle(new Vector3(0,1,0),profile.yaw);point.y+=190;point.project(camera)
...
for(const row of framing.filter(row=>!row.inside)){
 const camera=new PerspectiveCamera(55,row.aspect,profile.camera.near,24000),pose=orbitPose({x:0,y:190,z:0},0,{yaw:row.degrees*Math.PI/180,pitch:0},row.aspect,row.view,profile.camera)
 camera.position.copy(pose.position);camera.lookAt(pose.target);camera.updateMatrixWorld(true)
 let maxX=0,maxY=0
 ...
 for(let i=0;i<=120;i++){
  ...
  g.scene.traverse(mesh=>{if(!mesh.isMesh)return;for(let j=0;j<mesh.geometry.attributes.position.count;j++){
   mesh.getVertexPosition(j,vertex);vertex.applyMatrix4(mesh.matrixWorld).sub(center).multiplyScalar(scale);vertex.applyAxisAngle(new Vector3(0,1,0),profile.yaw);vertex.y+=190;vertex.project(camera)
...
console.log(JSON.stringify({id,profileRevision:profile.revision,path,sha256:createHash('sha256').update(source).digest('hex'),clip:clip.name,duration:clip.duration,samples:121,sourceBounds:envelope,initialCenter:center,scale,flightEnvelopeSize:envelope.getSize(new Vector3()).multiplyScalar(scale),rotationInvariantRadius:radius,framing},null,2))
