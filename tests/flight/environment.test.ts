...
import { terrainAt, coastAt } from '../../src/flight-experience/world'
describe('shared linear environment',()=>{
  ...
  it('publishes complete padded fields with a 520 query/frame bound and refreshes stationary morphs',()=>{
    ...
    expect(field.revision).toBe(2);expect(decodeHeight(field.data[0]!,field.data[1]!)).toBeCloseTo(2,1)
  })
  it('samples the actual shoreline, not the nominal coast function',()=>{
    ...
    expect(high-coastAt(z)).toBeCloseTo(133.551,2)
    const x0=Math.floor(high/16)*16,h0=terrainAt(x0,z).height,h1=terrainAt(x0+16,z).height
    const interpolated=x0+(-.7-h0)/(h1-h0)*16
