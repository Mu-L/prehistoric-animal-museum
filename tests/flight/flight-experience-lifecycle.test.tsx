...
describe('whole FlightExperience events with the actual Runtime (GPU/worker readiness stubbed)',()=>{
 it('keeps mode and movement on panel/rerender/language changes and isolates range keyboard shortcuts',async()=>{
  ...
  fireEvent.click(screen.getByRole('tab',{name:'How to fly'}))
...
it('preserves the selected progress but resets auto on Restart, and Defaults restores the original time',async()=>{
 ...
 fireEvent.click(screen.getByRole('button',{name:'Flight & scenery'}))
 fireEvent.click(screen.getByRole('tab',{name:'Stop and look'}))
...
it('groups scenery into three purpose tabs and reveals notes only by choice without stopping travel',async()=>{
 ...
 expect(screen.getByRole('tab',{name:'Scenery now'})).toHaveAttribute('aria-selected','true')
 expect(screen.getByRole('button',{name:'Automatic daylight'})).toBeVisible()
 expect(screen.getByRole('button',{name:'Light rain'})).toBeVisible()
 expect(screen.getByRole('checkbox',{name:'Vegetation breeze'})).toBeVisible()
 expect(screen.queryByRole('button',{name:/Light on water/})).toBeNull()
 const input=vi.spyOn(runtime.input,'key')
 fireEvent.keyDown(screen.getByRole('tab',{name:'Scenery now'}),{key:'ArrowRight',code:'ArrowRight'})
 expect(screen.getByRole('tab',{name:'Stop and look'})).toHaveFocus()
...
it('keeps a completed still and a global return command available across settings tabs during viewpoint preparation',async()=>{
 ...
 fireEvent.click(screen.getByRole('tab',{name:'Stop and look'}))
 fireEvent.click(screen.getByRole('button',{name:'Seaward · 120 m'}))
