...
describe('unified scenery sections',()=>{
 it('applies sunlight while flying without invoking pause or a viewpoint',()=>{
  ...
  expect(screen.queryByRole('button',{name:/Above the cliffs/})).not.toBeInTheDocument()
 })
 it('labels viewpoint interruption and retains scenery and return controls',()=>{
  ...
  expect(screen.getByText(/Flight pauses here/)).toBeVisible()
  fireEvent.click(screen.getByRole('button',{name:/Above the cliffs/}));expect(runtime.navigateViewpoint).toHaveBeenCalledWith('cliff')
  fireEvent.click(screen.getByRole('button',{name:'Pause scenery'}));expect(runtime.toggleScenery).toHaveBeenCalledOnce()
  expect(screen.queryByRole('button',{name:'Return to flight position'})).toBeNull()
 })
 it('keeps return available while a Chinese viewpoint is preparing',()=>{
  ...
  fireEvent.click(screen.getByRole('button',{name:/海崖俯瞰/}));expect(runtime.navigateViewpoint).toHaveBeenCalledWith('cliff')
 })
})
...
it('offers automatic daylight and an explicit replay only at sunset', () => {
 ...
 expect(runtime.setSolarDayProgress).toHaveBeenCalledWith(.68)
